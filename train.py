...
import time
import argparse     
from src.models.models import CERA
...
global logger
def get_args(seed, description='Multi-modal Multi-label Emotion Recognition'):
...
def init_model(args, device, n_gpu, local_rank): 
    ...
    # Prepare model
    model = CERA.from_pretrained(args.bert_model, args.visual_model, args.audio_model, args.cross_model, args.decoder_model, task_config=args)
...
def load_model(epoch, args, n_gpu, device, model_file=None):
    ...
    if os.path.exists(model_file):
        ...
        model = CERA.from_pretrained(args.bert_model, args.visual_model, args.audio_model, args.cross_model,
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0, label_input=None, label_mask=None): 
    ...
    for step, batch in enumerate(train_dataloader):
     #   torch.cuda.empty_cache()
        ...
        model_loss, batch_pred, true_label, pred_scores, cosine_loss, loss_av, loss_tv, loss_ta, adv_preivate_loss, adv_common_loss = model(pairs_text, pairs_mask, video, video_mask, audio, audio_mask, label_input, label_mask, groundTruth_labels=ground_label, training=True)
